    ...
    if(conf->kscan_passthrough){
        ...
        for (int j = 0; j < conf->bindings_len; j++) {
            ...
            int ret = zmk_behavior_invoke_binding(
                &conf->bindings[behavior_idx], behavior_event,
                pressed);
            if (ret < 0) {
...
static int rt_handle_event(const struct device *dev,
...
static int rt_init(const struct device *dev) {
    ...
    for(int i=0; i<conf->keymap_size; i++){
        data->key_states[i].last_value = 10000; // emulates the switch being at max height
...
#define RT_INIT(n)                                                        \
    ...
    rt_behaviors_bindings_##n[] = COND_CODE_1(DT_INST_NODE_HAS_PROP(n, bindings),({                          \
    ...
    DEVICE_DT_INST_DEFINE(n, &rt_init, NULL, &rt_data_##n,                    \
                          &rt_config_##n, POST_KERNEL,                    \
