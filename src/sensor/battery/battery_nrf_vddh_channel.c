...
static int vddh_sample_fetch(const struct device *dev,
    ...
    rc = adc_raw_to_millivolts(adc_ref_internal(adc), drv_data->acc.gain,
                               as->resolution, &val);
    if (rc != 0) {
    ...
    drv_data->value.state_of_charge =
        lithium_ion_mv_to_pct_(drv_data->value.millivolts);
...
static int vddh_channel_get(const struct device *dev, enum sensor_channel chan,
    ...
    return battery_channel_get_(&drv_data->value, chan, val);
}
...
static int vddh_init(const struct device *dev) {
    struct vddh_data *drv_data = dev->data;
    struct vddh_config *cfg = dev->config;
...
static struct vddh_config vddh_config = {
    .channel_id = DT_INST_PROP(0, vddh_channel),
};
